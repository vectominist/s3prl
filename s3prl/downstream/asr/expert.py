...
class DownstreamExpert(nn.Module):
    ...
    def __init__(
        ...
        if self.modelrc["select"] == "FrameLevel":
    ...
    def forward(self, split, features, labels, filenames, records, **kwargs):
        ...
        labels_len = torch.LongTensor([len(label) for label in labels]).to(
            device=device
        )
        labels = pad_sequence(
            labels,
        ...
        # HACK: CUDA illegal memory access issue
        # try:
        #     loss = self.objective(
        ...
        #     print("log_probs_len", log_probs_len.shape)
        #     print("labels_len", labels_len.shape)
        #     try:
        ...
        #     except:
        #         if self.training:
        #             tensor = torch.randn((10000, 10000, 1000), device=device)
        #         else:
    ...
    def log_records(
        ...
        try:
            ...
            pred_words = [
                normalizer(" ".join(sent)).split(" ") for sent in records["pred_words"]
            ]
            target_words = [
                normalizer(" ".join(sent)).split(" ")
                for sent in records["target_words"]
            ]
        ...
        save_names = []
        if split in ["dev-clean", "dt05_bth"] and wer < self.best_score:
            self.best_score = torch.ones(1) * wer
            save_names.append(f"{split}-best.ckpt")

        if "test" in split or "dev" in split or "dt05" in split or "et05" in split:
