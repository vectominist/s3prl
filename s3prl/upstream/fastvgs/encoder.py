...
class Wav2Vec2Model_cls(BaseFairseqModel):
    ...
    def __init__(self, args):
        super().__init__()
        self.args = args
        if not hasattr(args, "trim_mask"):
            self.args.trim_mask = False
    ...
    def apply_mask(self, x, padding_mask):
        ...
        if self.mask_prob > 0:
            mask_indices = self.Compute_Mask_Indices(
                ...
                require_same_masks=self.args.trim_mask,
            )
    ...
    def sample_negatives(self, y, mask_indices):
        ...
        negs = y[
            neg_idxs
        ]  # [T_pred1*n_negatives+T_pred2*n_negatives+...+T_predB*n_negatives, fsz]
        # negs = negs.view(
    ...
    def compute_preds(self, x, y, negatives, mask_indices):
        ...
        for i, mask_idx in enumerate(mask_indices):
            ...
            cur_negatives = (
                negatives[
                    accum * self.n_negatives : (accum + T_pred) * self.n_negatives, :
                ]
                .view(T_pred, self.n_negatives, -1)
                .transpose(0, 1)
            )
            ...
            logits = torch.cosine_similarity(
                cur_x.float(), targets.float(), dim=-1
            ).type_as(
                cur_x
            )  # this gives [N+1, T_pred]
            ...
            if self.negatives_from_everywhere:
                ...
                if self.args.trim_mask:
                    ...
                    negs, _ = self.sample_negatives(
                        y, mask_indices
                    )  # negs: [T_pred1+T_pred2+...+T_predB, D]
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        logging.info(f"Encoder layers = {args.encoder_layers}")
