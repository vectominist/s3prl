...
class AudioEncoder(BaseFairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--trim_mask", action="store_true", default=False)
        ...
        parser.add_argument("--use_audio_cls_token", action="store_true", default=False)

        parser.add_argument("--random_init_last_x", type=int, default=None)

        parser.add_argument("--freeze_first_x", type=int, default=None)
    ...
    def __init__(self, args):
        ...
        dropout = 0
        std = math.sqrt(
            (4 * (1.0 - dropout)) / (args.conv_pos * self.args.encoder_embed_dim)
        )
    ...
    def forward(
        ...
        ):  # resize padding mask according to Conv pooling ratio
        ...
        if need_attention_weights:
            features, attn_weights = self.encoder(
                x,
        ...
        if not superb:
            ...
            features = self.encoder(
                x, padding_mask=padding_mask, tgt_layer=tgt_layer, superb=True
            )
            return [feat_cnn] + features
            # return features
    ...
    def carefully_load_state_dict(self, states, load_all=False):
        ...
        if random_init_last_x != None and not load_all:
            ...
            assert cut >= 0
            random_init_names = [
                f"encoder.layers.{i}." for i in range(cut, self.args.encoder_layers)
            ]
            print(
                f"randomly reinitialize the weights start with the following: {random_init_names}\n"
            )
            ...
            if random_init_last_x != None and not load_all:
                for names in random_init_names:
                    if k.startswith(names):
                        v = torch.tensor([0.0]).to(
                            v.device
                        )  # make it so that the size doesn't match
                        break
    ...
    def get_last_selfattention(self, source, tgt_layer=None, padding_mask=None):
        ...
        if self.args.use_audio_cls_token:
            ...
            if padding_mask != None:
                ...
                padding_mask = torch.cat([cls_token_padding_mask, padding_mask], dim=1)
        # ############################################
        _, attn_weights = self.encoder.extract_features(
            x, padding_mask=padding_mask, need_head_weights=True, tgt_layer=tgt_layer
        )
        return attn_weights
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        if superb:
            ...
            all_feats = self.extract_features(
                x, padding_mask=padding_mask, all_hidden_states=True
            )
            return all_feats
        # print("1386", x.dtype)
        x, layer_feats = self.extract_features(
            x,
            padding_mask=padding_mask,
            tgt_layer=tgt_layer,
            need_head_weights=need_attention_weights,
            pre_feats=pre_feats,
        )
